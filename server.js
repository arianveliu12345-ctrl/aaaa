...
// One service deployment, many pages, one webhook URL
// ============================================
...
const ADMIN_USER = process.env.ADMIN_USER || 'YOUR_USERNAME_HERE';
const ADMIN_PASS = process.env.ADMIN_PASS || 'YOUR_PASSWORD_HERE';
...
function getDefaults() {
  return {
    redirectUrl: process.env.DEFAULT_REDIRECT_URL || 'https://example.com/landing',
    photos: (process.env.DEFAULT_PHOTOS
      ? process.env.DEFAULT_PHOTOS.split(',').map(s => s.trim()).filter(Boolean)
      : [
          'https://via.placeholder.com/600x600/4a90e2/ffffff?text=Photo+1',
          'https://via.placeholder.com/600x600/e94e77/ffffff?text=Photo+2',
          'https://via.placeholder.com/600x600/50c878/ffffff?text=Photo+3',
          'https://via.placeholder.com/600x600/f59e0b/ffffff?text=Photo+4'
        ]),
    title: process.env.DEFAULT_TITLE || 'Hello there 👋',
    subtitle: process.env.DEFAULT_SUBTITLE || 'Welcome to our page',
...
function addPage(data) {
  ...
  const newPage = {
    ...
    redirectUrl: data.redirectUrl || d.redirectUrl,
    photos: photos,
...
const DAILY_SUBTITLES = [
  ...
  "Have a look at what's new today",
  "We've got something for you 👋",
  "Take a look — fresh content",
...
// Click tracker — routes by pageId to the right redirect URL
...
app.get('/track', (req, res) => {
  ...
  const page = getPage(pageId);
  const dest = page ? page.redirectUrl : getDefaults().redirectUrl;
    ...
    <div class="card">
      ...
      <form action="/add-page" method="POST">
        <div class="row">
          <div>
            <label>Page ID *</label>
            <input name="pageId" required placeholder="e.g. 1234567890123456"/>
          </div>
          <div>
            <label>Page Label / Nickname</label>
            <input name="label" placeholder="e.g. My Page, Business"/>
          </div>
        </div>
        ...
        <details>
          ...
          <div class="row">
            ...
            <div>
              ...
              <input name="redirectUrl" placeholder="${esc(getDefaults().redirectUrl)}"/>
            </div>
          </div>
  ...
  return `<div class="container">
    ...
    <div class="card">
      ...
      <form action="/update-settings?page=${esc(page.pageId)}" method="POST">
        ...
        <div class="row">
          ...
          <div>
            <label>Redirect URL</label>
            <input name="redirectUrl" value="${esc(page.redirectUrl)}"/>
          </div>
        </div>
    ...
    <div class="card">
      ...
      <form action="/add-photo?page=${esc(page.pageId)}" method="POST" style="margin-top:14px;">
        ...
        <input name="photoUrl" placeholder="https://example.com/image.png"/>
...
// ============================================
// HEALTH CHECK (public — for hosting platform)
// ============================================
// Note: this is BEHIND the auth wall as written above. Most hosts don't need it.
// If your hosting platform insists on an unauthenticated /health, move this block above the auth wall.
...
app.listen(PORT, () => {
  ...
  console.log(`🌐 Public URL: ${PUBLIC_URL || '(set PUBLIC_URL environment variable)'}`);
  console.log(`🔒 Admin login: ${ADMIN_USER} / ${ADMIN_PASS === 'YOUR_PASSWORD_HERE' ? '⚠️  SET ADMIN_PASS ENVIRONMENT VARIABLE' : '(set)'}`);
  const pages = loadPages();
